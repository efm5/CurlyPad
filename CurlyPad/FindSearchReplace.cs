   ...
   public partial class CurlyPadForm : Form {
      ...
      private void ReplaceFirst() {
         ...
         if (replaceGlobalRadioButton.Checked) {
            ...
            textBox.Text = regex.Replace(textBox.Text, replacement, 1);
            sMatches = Regex.Matches(textBox.Text, pattern, regexOptions); //efm5 refresh after replacement
            sFindPosition = 0; //efm5 advance to first remaining match
         }
         else { //efm5 Selected
            Regex regex = new Regex(pattern, regexOptions);
            textBox.SelectedText = regex.Replace(textBox.SelectedText, replacement, 1);
            sMatches = Regex.Matches(textBox.SelectedText, pattern, regexOptions); //efm5 refresh after replacement
            sFindPosition = 0; //efm5 advance to first remaining match
         }
      }
